...
beforeAll(async () => {
  const path = fileURLToPath(new URL('../test/mocks', import.meta.url))
  await writeFile(
    resolve(path, 'importer.mjs'),
    `export { default as effect } from 'file://${path}/importee.mjs'
`,
  )
})
...
describe('pure annotation', () => {
  it('e2e', async () => {
    ...
    const filename = await readdir(assetsDir).then(files =>
